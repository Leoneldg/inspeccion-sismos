...
if (!puede('seguimiento', 'crear')) {
    flash('error', 'Solo un usuario con permiso de gestión puede definir el plan de acción.');
    header('Location: ' . $volver); exit;
}
...
// Si se marca "Culminada" manualmente, se fija la fecha fin real si falta.
// El avance NO se toca aquí: se calcula del consumo de recursos.
...
try {
    db()->prepare(
        'UPDATE seguimiento_obras SET
            ...
            tiempo_accion_dias = :dias, estado_obra = :eo,
        ...
        'dias' => $tiempoDias, 'eo' => $estadoObra,
        'pres' => $presupuesto, 'prio' => $prioridad, 'obs' => $observaciones,
        'uid' => $_SESSION['user_id'] ?? null, 'id' => $obraId,
    ]);

    // Recalcular el avance por si cambiaron los estimados (afecta el %).
    ...
    segBitacora($obraId, 'Plan de acción actualizado', null);
