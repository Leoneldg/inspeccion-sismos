...
// Parroquias asignadas (rol Responsable de Parroquia): llegan como arreglo.
$parrPost = $_POST['parroquias_asignadas'] ?? [];
if (!is_array($parrPost)) $parrPost = [];
$parroquiasAsignadas = implode(',', array_filter(array_map('trim', $parrPost)));
...
try {
    ...
    // Guardar las parroquias asignadas (columna opcional: se crea si falta).
    if ($id) {
        try {
            $cols = $pdo->query("SHOW COLUMNS FROM usuarios")->fetchAll(PDO::FETCH_COLUMN);
            if (!in_array('parroquias_asignadas', $cols, true)) {
                $pdo->exec("ALTER TABLE usuarios ADD COLUMN parroquias_asignadas VARCHAR(500) DEFAULT NULL");
            }
            $pdo->prepare('UPDATE usuarios SET parroquias_asignadas = :p WHERE id = :id')
                ->execute(['p' => ($esMaster ? null : ($parroquiasAsignadas ?: null)), 'id' => $id]);
        } catch (Throwable $e) { /* no bloquear el guardado del usuario */ }
    }
} catch (Throwable $e) {
