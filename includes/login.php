...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfValidar($_POST['csrf'] ?? null)) {
        ...
        if ($usuario === '' || $password === '') {
            ...
            if ($ok) {
                // Destino por defecto según el rol: el sistematizador de campo
                // cae en su panel de trabajo; los demás, en el inicio normal.
                $destino = APP_URL_BASE . 'index.php';
                if (function_exists('esSistematizador') && esSistematizador()
                    && !(function_exists('usuarioEsMaster') && usuarioEsMaster())) {
                    $rolNom = mb_strtolower($_SESSION['rol_nombre'] ?? '', 'UTF-8');
                    if (!str_contains($rolNom, 'administrador')) {
